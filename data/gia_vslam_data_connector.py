...
class GiaVSLAMdataConnector(DatasetConnector):
    ...
    def load_areas_from_building_structure(self):
        ...
        for floor_idx in range(self.num_floors):
            file_path = resource_filename("geo2wall", "kml/{}og.kml".format(self.floors[floor_idx]))

            df_walls = g2w.read_geometry_file(file_path, kml_folder="Waende")
            df_walls, origin, translation = g2w.rotate_and_translate(df_walls)
            df_rooms = g2w.read_geometry_file(file_path, kml_folder="Raeume")
            df, origin, translation = g2w.rotate_and_translate(df_rooms,
                                                               rotation_center=origin,
                                                               tranlation=translation)

            polys = [shapely.geometry.Polygon(p) for p in df.geometry]
            fp_polygons += [polys]

        self.fp_areas = fp_polygons
    ...
    def load_walls(self):
        ...
        for floor_idx in self.floors:
            # get walls from shapefiles
            file_path = resource_filename("geo2wall", "kml/{}og.kml".format(floor_idx))
            w_h, w_v = g2w.get_walls_from_geometry_file(file_path)
            walls_h.append(np.array(w_h))
            walls_v.append(np.array(w_v))

            # convert walls to linestrings
            walls = np.concatenate((w_h, w_v), axis=0)
            w_ls = [shapely.geometry.LineString(([w[0], w[1]], [w[2], w[3]])) for w in walls]
            walls_ls += [w_ls]

        self.walls_h, self.walls_v, self.walls_ls = walls_h, walls_v, walls_ls
